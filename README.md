...
![做成｜从任务到成品](apps/marketing/assets/og-cover.jpg)
...
做成服务大学生、研究生和刚进入职场的人。公开站可以直接选任务原型、生成行动路线；学生工作台负责把材料做成成果；运营后台负责课程、任务、用量和审计记录。

产品只守一条主线：**说清任务 → 留住原文 → 做出判断 → 组织表达 → 核验并带走成品**。每一步都有一个能检查的中间结果，离开后再回来也知道从哪里继续。
...
1. 打开主入口，先选“课程汇报、调研报告或答辩准备”中最接近的一项，把预填内容改成自己的真实任务，再生成行动路线。
...
首页先给出三种常见任务原型。原型只预填任务结构和验收提醒，不会伪造材料、证据或成果。确认真实任务名称、截止日期、交付形式、关键约束、用户阶段、当前重点和每日可投入时间后，页面会生成确定性的 7 步行动计划。阶段与重点会改变行动建议、故事画面和三条候选路线的排序；证据先行、结构先行、交付先行使用同一量尺展示价值与代价，用户明确确认后才能下载任务交付单。演练后的“节奏太慢”“信息太密”“证据不足”会进入下一轮并重新计算推荐、理由和第一步，而不是只显示一条反馈提示。
...
逐项实现、测试证据和缺口见[做成需求实现与证据](docs/zuocheng-requirements-evidence.md)；最终规格状态仍由[需求—证据台账](docs/requirements/zuocheng-requirements.md)判定。
...
- [做成需求实现与证据](docs/zuocheng-requirements-evidence.md)
