...
仓库记录的正式域名是 <https://zuocheng-ai-workflow.vercel.app/>。发布后应分别验证根路径、`/app/` 和 `/admin/`；部署记录必须以本次构建和运行时检查为准，README 不把历史部署自动视为最新版本。
...
首页任务启动台会根据任务名称、截止日期、交付形式和关键约束，生成确定性的 7 步行动计划。同一参考日期与相同输入会得到相同排期；不足 7 个自然日时会把多个步骤安排在同一天，已过去的截止日期会被拒绝。
