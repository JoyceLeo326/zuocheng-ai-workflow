...
> 从一项真实任务出发，把资料、证据、结构和初稿推进成可提交文件。

![做成■｜从模糊待办到可交付结果](apps/marketing/assets/og-cover.jpg)

做成■ 是面向大学生、研究生和初级职场用户的任务交付与学习平台。公开站介绍方法和课程，学生工作台用于处理自己的资料并完成交付，运营后台用于检查课程、任务、用量和审计记录。
...
首页任务启动台会根据任务名称、截止日期、交付形式、关键约束、用户阶段、当前重点和每日可投入时间，生成确定性的 7 步行动计划。阶段与重点会改变行动建议，进度会同步推动“起点—冲突—选择—结果”的任务故事。同一参考日期与相同输入会得到相同排期；不足 7 个自然日时会把多个步骤安排在同一天，已过去的截止日期会被拒绝。

- 计划由浏览器本地规则生成，并非 AI 模型输出，也不会向模型或远程接口发送任务内容。
- 任务字段和逐日完成状态保存在当前浏览器的 `localStorage`。
- “下载 Markdown”导出任务信息、每日动作、当日产出和已勾选进度。
- “重置当前任务”清空这份任务及其进度。
...
发布验收还需检查 `375×812`、`768×1024`、`1024×768`、`1440×900` 四个视口，键盘操作、离线重开、25 页 PDF、6 条证据、3 页初稿、核验、PPTX/PDF 导出和项目删除。自动测试通过不等于生产身份、外部 Provider 或跨设备流程已经成立。
